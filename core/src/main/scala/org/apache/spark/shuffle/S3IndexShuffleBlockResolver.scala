...
import org.apache.spark.internal.config.{SHUFFLE_S3_BUCKET_NAME, SHUFFLE_S3_PREFIX}
...
private[spark] class S3IndexShuffleBlockResolver(
  ...
  private val s3Bucket: String = conf.get(SHUFFLE_S3_BUCKET_NAME)
  private val s3Prefix: String = conf.get(SHUFFLE_S3_PREFIX)
    ...
    try {
      ...
      synchronized {
        ...
        if (existingLengths != null) {
          ...
          // Writing new index file to with custom_prefix/uuid/block manager uuid/subdir/name path
          ...
          val indexS3key: String = s3Prefix + uuid + "/" + blockMgrUuid + "/" +
          ...
          // Writing new data file to with custom_prefix/uuid/block manager uuid/subdir/name path
          ...
          val dataS3key: String = s3Prefix + uuid + "/" + blockMgrUuid + "/" +
