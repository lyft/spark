...
private[spark] class S3IndexShuffleBlockResolver(
                                                conf: SparkConf,
    ...
    try {
      ...
      synchronized {
        ...
        if (existingLengths != null) {
          ...
          // After successful renames
          // Writing new index file to to S3 with awen/uuid/block manager uuid/subdir/name path
          ...
          val blockMgrUuid: String = indexPathSplit(indexPathSplit.length - 3)

          val bucketName: String = "data-team"
          val indexS3key: String = "spark-shuffle-poc/" + uuid + "/" + blockMgrUuid + "/" + indexSubdir + "/" + indexFile.getName()
          try {
            s3Client.putObject(bucketName, indexS3key, indexFile)
            logInfo(s"Successfully wrote " + indexFile.getName() + " to S3 at awen/" + uuid)
          } catch {
          ...
          val dataS3key: String = "spark-shuffle-poc/" + uuid + "/" + blockMgrUuid + "/" + dataSubdir + "/" + dataFile.getName()

          try {
            s3Client.putObject(bucketName, dataS3key, dataFile)
            logInfo(s"Successfully wrote " + dataFile.getName() + " to S3 at awen/" + uuid)
          } catch {
            case ace: AmazonClientException => logError(ace.getMessage())
          }
        }
...
private[spark] object S3IndexShuffleBlockResolver {
